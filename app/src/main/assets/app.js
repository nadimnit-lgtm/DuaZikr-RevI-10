...
(function () {
  ...
  var DEFAULTS = {
    ...
    tvVisualRevision: "I-10"              // TV-only visual settings migration marker
  };
  ...
  function load() {
    ...
    try {
      ...
      if (IS_TV) {
        ...
        var needsVisualReset = raw.tvVisualRevision !== "I-10";
        if (needsVisualReset || !isFinite(rawAr) || rawAr < 1.65) { s.arScale = 1.75; repaired = true; }
        if (needsVisualReset || !raw.bismillahSize || raw.bismillahSize === "large" || raw.bismillahSize === "xl") { s.bismillahSize = "small"; repaired = true; }
        if (needsVisualReset && (!raw.theme || raw.theme === "dark-ambient" || raw.theme === "elder-light")) { s.theme = raw.theme || "dark-ambient"; }
        s.tvVisualRevision = "I-10";
      }
      ...
      if (IS_TV) { safe.arScale = 1.75; safe.bismillahSize = "small"; safe.tvVisualRevision = "I-10"; }
  ...
  function tvArabicFloor(mode) {
    if (!IS_TV) return 0;
    return ({ short: 86, normal: 78, long: 70, very_long: 64 })[mode] || 76;
  }
...
function autoSize(mode, scale) {
  ...
  var base;
  if (IS_TV) {
    // Rev I-10: default TV scale is deliberately high (1.65–1.85),
    // so base values are calibrated to land near the final CSS clamp.
    // Arabic remains the hero while long entries can still scroll respectfully.
    base = { short: 70, normal: 66, long: 58, very_long: 52 }[mode] || 64;
  } else {
  ...
  function render() {
    ...
    reader.style.setProperty("--ar-lh", s.arabicScript === "indopak" ? "1.72" : "1.34");
    var tvAdd = IS_TV ? 4 : 0;
...
function fitContent(arEl, arStart, tlStart, trStart) {
  ...
  var size = arStart;
  var wmin = IS_TV ? Math.max(tvArabicFloor(mode), Math.round(arStart * 0.72)) : Math.max(16, Math.round(arStart * 0.5));
  var guard = 0;
  ...
  if (autoFit) {
    var AR_MIN = Math.max(tvArabicFloor(mode), Math.round(arStart * 0.84)), TL_MIN = 18, TR_MIN = 18, tl = tlStart, tr = trStart, g = 0;
    var FILL = 0.985;
  ...
  function bindGlobal() {
    ...
    function stepArabic(d) {
      ensureSettings();
      var vMin = IS_TV ? 1.35 : 0.7;
      var vMax = IS_TV ? 2.2 : 2.0;
      var v = Math.min(vMax, Math.max(vMin, Math.round((state.settings.arScale + d) * 100) / 100));
      if (IS_TV) state.settings.tvVisualRevision = "I-10";
  ...
  function buildTvSettings() {
    ...
    body.appendChild(tvStepperRow("Arabic text size", "A− / A+ changes the live Arabic card size immediately.", "arScale", 0.1, 1.35, 2.2, true));
  ...
  function tvRowShell(name, desc, iconText) {
    ...
    var help = document.createElement("div"); help.className = "tv-set-help"; help.textContent = desc || "";
    r.appendChild(ic); r.appendChild(lab); r.appendChild(ctrl); r.appendChild(help);
...
function applyTvLivePreviewValues(model) {
  ...
  if (reader && it) {
    ...
    reader.style.setProperty("--tl-size", tl + "px");
    reader.style.setProperty("--tr-size", tr + "px");
    reader.style.setProperty("--ar-lh", model.arabicScript === "indopak" ? "1.72" : "1.34");
    var arEl = $("#mArabic");
    if (arEl) {
      arEl.style.fontSize = ar + "px";
      arEl.style.lineHeight = model.arabicScript === "indopak" ? "1.72" : "1.34";
...
function commitTvDraftLive(rebuildView, renderView) {
  ...
  state.draft.tvVisualRevision = "I-10";
  ...
  function applySettings() {
    ...
    if (IS_TV) state.draft.tvVisualRevision = "I-10";
    state.settings = Object.assign({}, state.draft);
